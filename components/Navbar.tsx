...
// Nav icon styles
const navIconClass = "w-4 h-4 text-gray-400 group-hover:text-primary transition-colors";
// Action button styles
const actionButtonClass = "p-2 sm:p-2.5 hover:bg-gray-50 rounded-2xl transition-colors relative text-gray-500 group";
// Search container styles
const searchContainerClass = "hidden lg:flex items-center gap-2 px-4 py-2 bg-gray-50 rounded-2xl border border-gray-100 focus-within:bg-white focus-within:ring-2 focus-within:ring-primary/10 transition-all";
// Sign in button styles
const signInButtonClass = "flex items-center gap-2 pl-2 pr-2 sm:pr-6 py-1.5 sm:py-2.5 bg-primary text-white rounded-[14px] sm:rounded-[18px] font-black hover:bg-primary-dark transition-all transform hover:-translate-y-0.5 active:translate-y-0 group shadow-lg shadow-primary/20";
    ...
    <header className="bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50 transition-all duration-300">
      ...
      <div className="container-custom px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-6 xl:gap-8">
          <NavItem href="/" title="Home" icon={<Home className={navIconClass} />} />
          <NavItem href="/products" title="Collection" icon={<LayoutGrid className={navIconClass} />} />
          <NavItem
            href="/products?sort=newest"
            title="New Arrivals"
            icon={<Zap className={navIconClass} />}
          />
          <NavItem href="/support" title="Support" icon={<LifeBuoy className={navIconClass} />} />
        </nav>
